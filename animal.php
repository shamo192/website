...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['search'])) {
        ...
        $sql = "SELECT * FROM animal_bite_record
                WHERE patient_name LIKE '%$search%' 
                OR address LIKE '%$search%' 
                OR guardian LIKE '%$search%' 
                OR guardian_address LIKE '%$search%' 
...
.dropdown-menu {
    ...
    left: auto; /* Prevents it from stretching to the left */
}
            ...
            <nav class="sb-sidenav accordion sb-sidenav-dark" id="sidenavAccordion">
                <div class="sb-sidenav-menu">
                    <div class="nav">
                        ...
                        <a class="nav-link collapsed" href="#" data-bs-toggle="collapse" data-bs-target="#collapsePages" aria-expanded="false" aria-controls="collapsePages">Reports for animal bites</a>
                        <div class="collapse" id="collapsePages" aria-labelledby="headingTwo" data-bs-parent="#sidenavAccordion">
                            <nav class="sb-sidenav-menu-nested nav accordion" id="sidenavAccordionPages">
                                ...
                                <a class="nav-link" href="animalNPyearly.php">New animal bites yearly/Monthly report</a>
                                <a class="nav-link" href="animalRPyearly.php">Retreat animal bites yearly/Monthly report</a>
                                <a class="nav-link" href="animalchart.php">Animal bites yearly report</a>
                            </nav>
                        </div>
                    </div>
                </div>
        ...
        <div id="layoutSidenav_content">
            <main>
                <div class="container-fluid px-4">
                    <h1 class="mt-4">Barangay Raniag animal record</h1>

                    <!-- Search Form -->
                    <form method="POST" action="">
                        <div class="input-group mb-3">
                            <input type="text" class="form-control" name="search" placeholder="Search for animal bites patient..." aria-label="Search">
                            ...
                            <table>
                                ...
                                <tbody>
                                    <?php foreach ($results as $row) { ?>
                                        <tr>
                                            <td data-label="Patient ID"><?php echo htmlspecialchars($row['bite_id']); ?></td>
                                            <td data-label="Name"><?php echo htmlspecialchars($row['patient_name']); ?></td>
                                            ...
                                            <td data-label="Birthdate"><?php echo htmlspecialchars($row['birthdate']); ?></td>
                                            <td>
                                                <a href="print.php?bite_id=<?php echo urlencode($row['bite_id']); ?>" class="icon-link">
                                                    <i class="fa-solid fa-print"></i>
                                                </a>
                                                <a href="viewanimal.php?bite_id=<?php echo urlencode($row['bite_id']); ?>" class="icon-link">
